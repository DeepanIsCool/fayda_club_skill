...
export async function GET(req: NextRequest) {
  ...
  try {
    const apiRes = await fetch(`${process.env.BASE_URL}/users`, {
      method: "GET",
      headers: {
    ...
    console.error("Error fetching all users from API:", err);
    return NextResponse.json(
      { success: false, error: "Failed to fetch all users from API" },
      { status: 500 }
    );
  }
}
