...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Try to parse response as JSON; if it fails but status is OK, still return success
      let data: unknown = null;
      try {
        data = await response.json();
      } catch (parseErr) {
        console.warn('⚠️ External gamesession response was not JSON-parsable');
      }

      if (!response.ok) {
        console.error('❌ External gamesession API error:', response.status, data);
        // Do not fail client submissions in case the external API is down; acknowledge locally
        return NextResponse.json(
          {
            success: true,
            message: 'Session recorded locally (external API unavailable)',
      ...
      return NextResponse.json(
        data ?? { success: true, message: 'Session submitted' },
        { status: 200 },
      );
    } catch (externalError) {
      console.error('❌ Error calling external gamesession API:', externalError);
      // Graceful fallback
      return NextResponse.json(
        {
          success: true,
          message: 'Session recorded locally (external API error)',
        },
        { status: 200 },
      );
    }
    ...
    // If we fail before contacting external API (e.g., invalid body), surface a clear message
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to process game session request' },
      { status: 400 },
    );
  }
}
